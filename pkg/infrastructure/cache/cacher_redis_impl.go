...
func (rs redisCacherImpl) HashSet(
	...
	if err := rs.Client.HSet(c, key, field, value, expire).Err(); err != nil {
		rs.log.Error("Error while trying to set value", err.Error())
		// print error and retry one more time then exit if fail.
		if err = rs.Client.HSet(c, key, value, expire).Err(); err != nil {
			return fmt.Errorf("failed to set hash key: %w", err)
		}
	}
	return nil
}
